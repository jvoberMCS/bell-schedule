...
export const getFontHeight = (ctx: CanvasRenderingContext2D, str: string) => {
    return ctx.measureText(str).fontBoundingBoxAscent + ctx.measureText(str).fontBoundingBoxDescent
}
export const getCanvasDimensions = (canvas: HTMLCanvasElement) => {
    return {
        w: canvas.width,
        h: canvas.height,
        landscape: canvas.width > canvas.height ? true : false,
    }
}
...
export const DrawSchedule = (
    ...
    if (ctx !== null && canvas !== null) {
        const can = getCanvasDimensions(canvas)
        const chunkOfDay = getChunkOfDay(now, schedule);

        // Set text settings
        ctx.font = '2rem Fira Code';
        ...
        modBells.forEach((modBell, i) => {
            ...
            const xPosStr1 = can.landscape === true ? (can.w / 2) : can.w * 0.1
            ...
            // Draw Mod Name
            ctx.textAlign = 'left';
            ctx.fillText(str1, xPosStr1, yLocation);
...
export const DrawNextEndOfMod = (
    ...
    if (ctx !== null && canvas !== null) {
        ...
        const chunkOfDay = getChunkOfDay(now, schedule);
        ctx.font =
            chunkOfDay === 'Class Change' ? can.landscape === true ? '3.5rem Fire Code' : '4rem Fira Code' : '2em Fira Code';
        ctx.textAlign = 'center';
        ...
        ctx.miterLimit = 2; // Gets rid of glitches
        ctx.fillText(str, x, y)
        // Multi-line class change display
        if (chunkOfDay === 'Class Change') { ctx.fillText(str2, x, y + getFontHeight(ctx, 'T')) }
    }
};
...
export const DrawTimeLeftInDay = (
    ...
    if (ctx !== null && canvas !== null) {
        ...
        const str1 = `${chunkOfDay === 'After School' || chunkOfDay === 'Student Dismissal'
        ...
        const str2 = `${chunkOfDay === 'After School' || chunkOfDay === 'Student Dismissal'
